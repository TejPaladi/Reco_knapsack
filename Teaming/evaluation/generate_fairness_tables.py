"""
Generate fairness tables for Teaming in the style of the fairness paper.

This script focuses on gender representation in recommended teams:
- Table 2 compares the inferred gender distribution of people appearing in the
  top recommended team for each aligned (proposal_id, researcher_name) row
  against the inferred researcher-population baseline.
- Table 3 evaluates group fairness on member-selection decisions. For each
  aligned row, the oracle label is the union of members appearing in the
  highest-goodness top team(s) across methods; each method predicts the members
  in its own top team.

When `--name-inference-mode=paper_ensemble`, gender is inferred from first names
using a paper-style ensemble of `gender-guesser`, `Genderize`, and
`SexMachine`, with biography-pronoun fallback when the name ensemble stays
ambiguous. When `--paper-subset` is enabled, the script approximates the paper's
smaller Teaming slice by keeping the first N proposals and first K anchor
researchers by appearance before computing the fairness tables.
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "data" / "v1_output_teaming" / "teaming_1698proposals_316researchers"),
        help="Directory containing the canonical UC1 Teaming CSVs.",
    )
    parser.add_argument(
        "--researchers-file",
        default=str(PROJECT_ROOT / "data" / "v1_input_files" / "v1_researchers.csv"),
        help="Primary researcher roster with names and descriptions.",
    )
    parser.add_argument(
        "--faculty-file",
        default=str(PROJECT_ROOT / "data" / "v1_input_files" / "v1_og_faculty.csv"),
        help="Supplementary faculty file used for pronoun fallback.",
    )
    parser.add_argument(
        "--results-dir",
        default=str(PROJECT_ROOT / "evaluation" / "output_paper_subset"),
        help="Directory where fairness tables and support artifacts will be saved.",
    )
    parser.add_argument(
        "--name-inference-mode",
        choices=["basic", "paper_ensemble"],
        default="paper_ensemble",
        help="How to infer gender from researcher first names.",
    )
    parser.add_argument(
        "--conditioning-attribute",
        default="year",
        help="Row attribute used for CSP conditioning.",
    )
    parser.add_argument(
        "--paper-subset",
        action="store_true",
        dest="paper_subset",
        help="Use the paper-style smaller Teaming slice by keeping the first N proposals and first K anchor researchers by appearance.",
    )
    parser.add_argument(
        "--full-dataset",
        action="store_false",
        dest="paper_subset",
        help="Disable the default paper-style subset and run on the full aligned Teaming dataset.",
    )
    parser.add_argument(
        "--subset-proposals",
        type=int,
        default=100,
        help="Proposal count used when --paper-subset is enabled.",
    )
    parser.add_argument(
        "--subset-anchor-researchers",
        type=int,
        default=46,
        help="Anchor researcher count used when --paper-subset is enabled.",
    )
    parser.add_argument(
        "--round-digits",
        type=int,
    ...
    parser.set_defaults(paper_subset=True)
...
def load_profiles(researchers_file: Path, faculty_file: Path) -> dict[str, list[str]]:
    profiles: dict[str, list[str]] = defaultdict(list)

    researchers = pd.read_csv(researchers_file)
    for _, row in researchers.iterrows():
        pieces = [
            str(row.get("descriptions", "")),
            str(row.get("research", "")),
            str(row.get("titles", "")),
        ]
        profiles[normalize_name(str(row["names"]))].append(" ".join(piece for piece in pieces if piece and piece != "nan"))
    ...
    for _, row in faculty.iterrows():
        ...
        profiles[normalize_name(str(row["name"]))].append(" ".join(piece for piece in pieces if piece and piece != "nan"))
...
def load_method_frame(output_dir: Path, filename: str, method_label: str, method_name: str) -> pd.DataFrame:
    ...
    keep = df[["proposal_id", "researcher_name", "year", f"{method_label}_top_team", f"{method_label}_top_goodness"]].copy()
    keep["researcher_name_norm"] = keep["researcher_name"].map(normalize_name)
    keep = keep.rename(columns={"year": f"{method_label}_year"})
...
def build_aligned_rows(output_dir: Path) -> tuple[pd.DataFrame, dict[str, dict[str, str]]]:
    ...
    for frame in frames[1:]:
        aligned = aligned.merge(
            frame,
            on=["proposal_id", "researcher_name", "researcher_name_norm"],
            how="inner",
        )

    year_cols = [column for column in aligned.columns if column.endswith("_year")]
    aligned["year"] = aligned[year_cols].bfill(axis=1).iloc[:, 0]
    aligned = aligned.drop(columns=year_cols)
    return aligned, meta


def apply_paper_subset(aligned: pd.DataFrame, proposal_count: int, researcher_count: int) -> tuple[pd.DataFrame, dict[str, int]]:
    proposal_ids = pd.Index(aligned["proposal_id"]).drop_duplicates().tolist()[:proposal_count]
    proposal_filtered = aligned[aligned["proposal_id"].isin(proposal_ids)].copy()
    anchor_researcher_norms = (
        pd.Index(proposal_filtered["researcher_name_norm"]).drop_duplicates().tolist()[:researcher_count]
    )
    subset = proposal_filtered[proposal_filtered["researcher_name_norm"].isin(anchor_researcher_norms)].copy()
    if subset.empty:
        raise ValueError("Paper subset filtering removed every aligned row.")
    return subset, {
        "requested_proposals": proposal_count,
        "actual_proposals": int(subset["proposal_id"].nunique()),
        "requested_anchor_researchers": researcher_count,
        "actual_anchor_researchers": int(subset["researcher_name_norm"].nunique()),
        "description": (
            "Post-hoc approximation of the paper's smaller Teaming slice: keep the first "
            "proposals and first anchor researchers by appearance in the aligned output."
        ),
    }
...
def main():
    ...
    if args.paper_subset:
        aligned, subset_meta = apply_paper_subset(aligned, args.subset_proposals, args.subset_anchor_researchers)

    profiles = load_profiles(Path(args.researchers_file), Path(args.faculty_file))
    ...
    metadata = {
        ...
        "gender_inference": {
            ...
            "description": (
                ...
                "gender-guesser, Genderize, and SexMachine, with biography-pronoun "
                "fallback when the name-only ensemble remains ambiguous."
                if args.name_inference_mode == "paper_ensemble"
                else "Gender is inferred from gender-guesser, with biography-pronoun fallback."
            ),
        },
        "table2_interpretation": (
            "Table 2 compares the inferred gender distribution of researchers appearing in each "
            "method's top recommended team against the inferred researcher-population baseline."
        ),
        "table3_interpretation": (
            "Table 3 operates on member-selection decisions. For each aligned "
            "(proposal_id, researcher_name) row, the oracle label is the union of members in the "
            "highest-goodness top team(s) across methods; each method predicts the members in its "
            "own top team. Add-0.5 smoothing is applied to all rate-based metrics."
        ),
        "table3_metrics": {
    ...
    if subset_meta is not None:
        metadata["paper_subset"] = subset_meta
