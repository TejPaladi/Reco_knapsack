...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Teaming M6 as a pure exact knapsack-style baseline: "
...
def load_proposal_pseudo_skills(path: Path) -> dict[str, dict[str, set[str]]]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            mapping[row["nsf_proposal_links_v1"]][row["researcher"]] = set(parse_literal_skills(row["pseudo_skills"]))
    return mapping
...
def load_proposal_skills(path: Path) -> dict[str, list[str]]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            mapping[row["nsf_proposal_links_v1"]] = parse_literal_skills(row["skills"])
    return mapping
...
def main() -> None:
    ...
    for proposal_index, (proposal_link, proposal_rows) in enumerate(rows_by_proposal.items(), start=1):
        ...
        for row in proposal_rows:
            ...
            for goodness, team in scored_variants:
                teaming_rows.append(
                    {
                        "nsf_proposal_links_v1": proposal_link,
                        "researcher_name": anchor,
                        "team": team,
                    }
                )
    ...
    write_rows(
        ...
        ["nsf_proposal_links_v1", "researcher_name", "team"],
        teaming_rows,
    )
    write_rows(
    ...
    metadata = {
        "method": "M6",
        "description": (
            "Exact anchor-fixed knapsack baseline on top of the M1 lexical matching pipeline. "
